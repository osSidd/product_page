...
export default function FilterSection({products, productQty, view, sortValue, toggleView, toggleSort, toggleProductQty}){
    ...
    return (
        <aside className="flex items-center justify-between px-6 py-4 my-4 bg-gray-100 ">
            <p className="md:text-lg font-normal">{products.length} items</p>
            <form action="" className="">
                <label htmlFor="" className="md:text-lg font-normal capitalize">sort by
                    <select name="" id="" value={sortValue} onChange={toggleSort} className=" capitalize cursor-pointer md:mx-8" >
                        {
                            sortOptions.map(option => (
                                <option key={option} value={option} className="md:text-lg font-normal capitalize">{option}</option>
                            ))
                        }
                    </select>
                </label>
                <label htmlFor="" className="md:text-lg font-normal capitalize ml-6 md:ml-16">show
                    <select onChange={toggleProductQty} name="" id="" value={productQty} className="md:text-lg px-4 font-normal md:mx-8 cursor-pointer">
                        {
                            qtyOptions.map(option => (
                                <option key={option} value={option} className="md:text-lg font-normal cursor-pointer">{option}</option>
                            ))
                        }
                    </select>
                </label>
